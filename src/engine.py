...
This module contains the core BusinessSimulator class that models a retail business
with full double-entry accounting and perpetual inventory tracking.

The simulator accurately models:
- Sales demand with trends, seasonality, and price sensitivity
- Multi-vendor supply chain with lead times and shipping
- Double-entry financial ledger (Assets = Liabilities + Equity)
- Perpetual inventory system with running balances
- Marketing campaigns and market events
...
class BusinessSimulator:
    """
    Main simulation engine for Digital Harvest.

    This class manages the entire business simulation, including:
    - State persistence (cash, date)
    - Inventory management and ordering
    - Sales processing with dynamic demand
    - Financial transactions and accounting
    - Market events and product unlocks

    Attributes:
        cash (Decimal): Current cash on hand
        current_date (datetime): Current simulation date/time
        simulation_start_date (datetime): Date the business started
        unlockables (list): Products that unlock at revenue milestones
        event_templates (list): Possible market events that can occur
    """
    def __init__(self):
        self._load_state()
        self.unlockables = [
    ...
    def _load_state(self):
        """
        Load the current business state from the database.

        Retrieves cash on hand, current date, and start date from the
        business_state table. Sets default values if no state exists.
        """
    ...
    def _save_state(self):
        """
        Persist the current business state to the database.

        Saves cash on hand and current date using INSERT ... ON DUPLICATE KEY UPDATE
        to handle both new and existing state records.
        """
        conn, cursor = self._get_db_connection()
