"""
Digital Harvest - Flask REST API

This module provides a REST API for the Digital Harvest business simulator.
It exposes endpoints for:
- Retrieving business status and data
- Placing orders and advancing time
- Managing products, marketing, and loans

The API uses Flask and Flask-CORS to serve data to the web interface.
...
class CustomEncoder(json.JSONEncoder):
    ...
    Converts Decimal to float and datetime to ISO 8601 strings for JSON serialization.
    """
...
# Initialize the business simulator (singleton instance for all requests)
try:
...
def check_sim(func):
    """
    Decorator to verify simulator is initialized before handling requests.

    Returns a 500 error if the simulator failed to initialize (e.g., database unavailable).
    """
...
@check_sim
def get_status():
    """Get current business status (cash, date, inventory value, debts)."""
