...
def get_backup_dir():
    """Get cross-platform Documents/DigitalHarvest_Data path"""
    if sys.platform == 'win32':
        docs = Path(os.environ.get('USERPROFILE', str(Path.home()))) / 'Documents'
    else:  # macOS/Linux
        docs = Path.home() / 'Documents'

    backup_dir = docs / 'DigitalHarvest_Data'
...
def backup_database(db_path):
    """
    Backup database to Documents/DigitalHarvest_Data
    - Always copy to digitalharvest.db (latest)
    - Daily: digitalharvest_YYYY-MM-DD.db (keep 3)
    - Weekly: digitalharvest_week-NN.db on Sundays (keep 4)
    """
    ...
    shutil.copy2(db_path, backup_dir / 'digitalharvest.db')

    # 2. Daily backup (once per day)
    daily_file = daily_dir / f"digitalharvest_{today.strftime('%Y-%m-%d')}.db"
    ...
    if today.weekday() == 6:  # Sunday
        ...
        weekly_file = weekly_dir / f"digitalharvest_week-{week_num:02d}.db"
...
def cleanup_old_daily_backups(daily_dir, days=3):
    ...
    for f in daily_dir.glob('digitalharvest_*.db'):
        try:
            # Parse date from filename
            date_str = f.stem.replace('digitalharvest_', '')
...
def cleanup_old_weekly_backups(weekly_dir, weeks=4):
    ...
    files = sorted(weekly_dir.glob('digitalharvest_week-*.db'), reverse=True)
...
def find_latest_backup():
    ...
    try:
        backup_dir = get_backup_dir()
        latest = backup_dir / 'digitalharvest.db'
...
def setup_database():
    ...
    db_path = Path(__file__).parent / "src" / "data" / "digitalharvest.db"
    ...
    if db_path.exists():
        ...
        if success:
            print("[OK] Saved to Documents/DigitalHarvest_Data")
        else:
...
def start_flask_server():
    ...
    print("[6/6] Starting Digital Harvest server...")
    print()
    print("=" * 60)
...
def main():
    ...
    print()
    print("=" * 60)
    print("Digital Harvest - Business Simulation Game")
    print("=" * 60)
    print()
    ...
    try:
        ...
        print()
        print("=" * 60)
        print("Server stopped. Thank you for playing Digital Harvest!")
        print("=" * 60)
        print()
